"""Replay HK backtest trades with explicit entry and partial-exit rules.

港股没有涨跌停，价格不连续风险主要来自合股/拆股/供股（而非美股的极端单日暴涨暴跌），
且仙股/流动性陷阱是港股特有的信号污染源，因此复用 core.hk_risk_filter 做统一风控，
而不是照搬美股的拆股启发式判定。策略规则本身（Wyckoff SOS/EVR/LPS 触发 + 分批止盈）
与市场无关，保持和美股版本一致的六策略矩阵，便于跨市场横向对比。
"""
...
def _signal_triggers(stats: dict[str, float]) -> tuple[list[str], float]:
    ...
    if stats["pct"] >= 7.0 and stats["vol_ratio"] >= 3.0 and stats["breakout_ratio"] >= 0.98:
        triggers.append("SOS")
        score += 30.0
...
def _has_risk_blocked_window(candles: pd.DataFrame, start_idx: int, end_idx: int) -> bool:
    return any(_hk_risk_blocked(candles, pos) for pos in range(max(1, start_idx), min(end_idx, len(candles) - 1) + 1))
...
def _collect_symbol_candidates(
    ...
    for row_idx, row in features[mask].dropna().iterrows():
        if _hk_risk_blocked(candles, int(row_idx)):
            continue
...
def _replay_one(row: dict[str, Any], hist_map: dict[str, pd.DataFrame], strategy: StrategySpec) -> ReplayTrade | None:
    ...
    if entry is None:
        if _has_risk_blocked_window(candles, idx + 1, min(idx + strategy.max_days, len(candles) - 1)):
            return None
        return _unfilled_trade(row, candles, idx, strategy, base_price)
    buy_idx, buy_price = entry
    if _has_risk_blocked_window(candles, idx + 1, min(buy_idx + strategy.max_days, len(candles) - 1)):
        return None
...
def _summary(strategy: StrategySpec, trades: list[ReplayTrade], period: dict[str, str], top_n: str) -> dict[str, Any]:
    ...
    return {
        "source": "local_hk_strategy_replay",
...
def run_hk_strategy_replay(args) -> int:
    ...
    print(f"[hk-replay] input signals={len(rows)}")
    ...
    for strategy in STRATEGIES:
        ...
        print(f"[hk-replay] {strategy.name}: trades={len(trades)}, sharpe={summary.get('sharpe_ratio')}")
    return 0
