"""HK backtest report aggregation and Feishu card builders."""
...
@dataclass(frozen=True)
class HkBacktestNotifyRequest:
    artifacts_dir: str = "artifacts"
    output: str = "docs/BACKTEST_HK_REPORT.md"
    run_url: str = ""
    top_n: str = "2"
    webhook_url: str = ""


def run_hk_backtest_notification(request: HkBacktestNotifyRequest) -> int:
    cells = load_cells(Path(request.artifacts_dir))
    print(f"[hk-backtest-notify] loaded {len(cells)} summary.json cells")
    write_report(Path(request.output), cells, run_url=request.run_url, top_n=str(request.top_n))
    webhook = str(request.webhook_url or "").strip() or os.getenv("FEISHU_WEBHOOK_URL", "").strip()
    if webhook:
        send_feishu(webhook, build_card(cells, run_url=request.run_url, top_n=str(request.top_n)))
    else:
...
def load_cells(artifacts_dir: Path) -> list[HkBacktestCell]:
    cells = []
...
def _rank_key(cell: HkBacktestCell) -> tuple[float, float, int]:
...
def _group_by_period(cells: list[HkBacktestCell]) -> dict[str, list[HkBacktestCell]]:
    grouped: dict[str, list[HkBacktestCell]] = defaultdict(list)
    for cell in cells:
        grouped[cell.period_key].append(cell)
    return dict(grouped)


def _period_title(cells: list[HkBacktestCell]) -> str:
    first = cells[0]
...
def _period_elements(cells: list[HkBacktestCell]) -> list[dict[str, Any]]:
...
def _strategy_row(cell: HkBacktestCell, best: HkBacktestCell | None) -> dict[str, Any]:
...
def _strategy_notes(cells: list[HkBacktestCell]) -> dict[str, Any]:
...
def build_card(cells: list[HkBacktestCell], *, run_url: str, top_n: str) -> dict[str, Any]:
    elements: list[dict[str, Any]] = [
        {
            "tag": "div",
            "text": {
                "tag": "lark_md",
                "content": f"**市场** HK  ·  **TopN** {top_n}  ·  **共 {len(cells)} 单元**\n[查看 GitHub Actions 详情]({run_url})",
            },
        }
    ]
    ...
    return {
        ...
        "card": {
            "header": {
                "title": {"tag": "plain_text", "content": "📊 HK Backtest Grid 港股回测完成"},
                "template": template,
            },
            "elements": elements,
        },
...
def write_report(path: Path, cells: list[HkBacktestCell], *, run_url: str, top_n: str) -> None:
    lines = ["# HK Backtest Strategy Comparison", "", f"- TopN: {top_n}", f"- GitHub Actions: {run_url or '-'}", ""]
