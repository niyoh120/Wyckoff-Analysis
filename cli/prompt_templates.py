...
BUILTIN_PROMPT_TEMPLATES: dict[str, PromptTemplate] = {
    "daily": PromptTemplate(
        ...
        prompt=(
            ...
            '4. 调用 query_history(source="attribution", limit=1) 查看策略治理器、升降权建议、latest_execution_state 和 latest_operations。\n'
            '5. 如果用户提到持仓或当前组合，再调用 portfolio(mode="view")；没有提到就不要主动诊断持仓。\n'
            "6. 输出：市场状态、今日可观察方向、shadow 最新新增/移除样本、归因调权当前影响范围、需要回避的风险、下一步动作。"
        ),
    ),
