...
- 修复 TUI 流式渲染清理：按 RichLog 实际 visual strips 删除临时输出，避免屏幕残留内容多于对话日志。
- 持仓/体检强制工具调用回归测试保持覆盖，降低模型只输出计划、不执行工具的复发风险。

### 运维

- 移除盘中持仓监控 GitHub Actions 任务及其配套脚本，避免与现有 Tail Buy / 持仓诊断能力重叠。
