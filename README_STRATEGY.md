...
- **CRASH**：崩盘日默认禁止新开仓，并启用 **抗跌观察车道 (crash_resilience_watch)**。只有此前已进入左侧观察来源、盘中跌破支撑后收回且高收承接的 Top1 候选，才切成 `CRASH_LEFT_PROBE`。
...
历史回测会额外输出 `CRASH_LEFT_PROBE` 分段仓位诊断，不把观察候选混入普通成交样本。该诊断以日线做近似代理：D 日盘后产生 `crash_resilience_watch`，D+1 低点跌破支撑后收回且收位不低于 0.65 时按收盘价代理 2% 试错，D+2 复用正式状态机确认后才模拟增量 3%，同日只取 Top1。报告披露权重收益和，不把它冒充组合收益。该口径仍含 D+1 同收盘判定/成交假设、不计现金组合佣金，也无法验证 VWAP、分钟 Spring 质量和尾盘冲高回落，因此只用于判断分段仓位规则是否值得继续保留，不能替代分钟线成交回放。
...
无论哪种模式，`confirmed`、数据质量、市场闸门和候选护栏都由程序控制。AI 对外部新仓输出的 `ATTACK` 会被程序降为 `PROBE`；AI 不能扩大规则授予的风险权限。尾盘默认 `TAIL_BUY_AI_POLICY=veto_only`，模型只能把规则结论降级或否决，不能把 `WATCH` 升级为 `BUY`；`shadow` 只记录模型结论，不改变规则决策。
