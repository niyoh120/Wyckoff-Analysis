...
- **PANIC_REPAIR_CONFIRMED**：候选次日通过指数价格与全市场广度确认，最多开放一只5%小额 PROBE，禁止 ATTACK

恐慌修复不是单日反弹标签，而是三日状态机：`CRASH → PANIC_REPAIR → PANIC_REPAIR_CONFIRMED`。候选日要求指数达到反弹阈值、上涨家数占比至少60%且涨跌幅中位数为正；次日要求指数不跌、上涨家数占比至少50%且中位数不为负。任一条件不满足都不开放试探仓。
...
Step3 可生成研报不等于次日可买。信号状态机要求 SOS / EVR 确认时守住关键支撑并站稳 MA20；未确认为 `pending`，跌破结构则 `expired`。即使 Step3 无候选或主模型不可用，系统仍会输出空集/回退报告、合规摘要与明确的次日行动，不把“AI 未运行”误写成“策略故障”。
...
当大盘 CRASH、RISK_ON、PANIC_REPAIR 或盘前风险 RISK_OFF/BLACK_SWAN 时，冻结**新开仓**权限（旧仓仍可管理）。`PANIC_REPAIR_CONFIRMED` 仅解除小额 PROBE 权限，不等同于恢复正常交易。
