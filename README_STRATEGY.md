...
系统先用全市场数据发现 A 股主线与候选，再用跨日确认过滤买点，最后交给 AI 复核和 OMS；回测证实次日开盘价入场优于分时尾盘择时，无需额外盘中执行任务。

**实盘怎么用（先读）**：[docs/OPERATOR_PLAYBOOK.md](docs/OPERATOR_PLAYBOOK.md)

- 日漏斗 = 候选与环境；次日开盘价附近买入 = 今天能不能买（串联，非二选一）
- 只买：环境允许 × 主线优先 × confirmed × 次日开盘价附近买入；唯一例外是满足黄金坑硬条件的 `CRASH_LEFT_PROBE`
...
3. **先入视野，再等买点** — 漏斗候选 ≠ 可买；confirmed 后按次日开盘价附近买入才执行
