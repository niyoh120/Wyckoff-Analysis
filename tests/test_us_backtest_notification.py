...
from workflows.us_backtest_notification import (
    UsBacktestNotifyRequest,
    build_card,
    load_cells,
    run_us_backtest_notification,
    write_report,
)
...
def test_build_card_includes_best_strategy() -> None:
    ...
    assert "US Backtest Grid 美股回测完成" in content
...
def test_write_report_outputs_strategy_table(tmp_path) -> None:
    ...
    assert "# US Backtest Strategy Comparison" in report
    assert "| 策略 | 说明 | 夏普 | 胜率 | 均收 | 回撤 | 样本 |" in report
    assert "策略2" in report


def test_run_us_backtest_notification_writes_report_and_sends_card(monkeypatch, tmp_path) -> None:
    sent: list[dict] = []
    output = tmp_path / "report.md"
    monkeypatch.setattr(
        "workflows.us_backtest_notification.send_feishu",
        lambda webhook, payload: sent.append({"webhook": webhook, "payload": payload}),
    )

    result = run_us_backtest_notification(
        UsBacktestNotifyRequest(
    ...
    assert "US Backtest Grid 美股回测完成" in json.dumps(sent[0]["payload"], ensure_ascii=False)
