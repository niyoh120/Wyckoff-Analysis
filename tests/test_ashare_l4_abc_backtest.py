...
from scripts.backtest_runner import _apply_abc_filter
...
def test_abc_filter_keeps_codes_with_two_or_more_conditions(monkeypatch):
    ...
    monkeypatch.setattr("scripts.backtest_runner.score_springboard_abc", fake_score)

    passed = _apply_abc_filter(
        ["A", "B", "C", "D"],
