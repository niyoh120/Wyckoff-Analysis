...
6. 策略归因问题必须调用 query_attribution，先确认返回结果是否来自远端表或提示本地 --no-write 报告，再读取 latest_execution_state、next_action、promotion_status、promotion_checklist 和 latest_operations 后判断信号升降权、是否能晋级 dynamic=on，以及 shadow 新增/移除样本。`
...
function buildReadTools(deps: ToolDeps, userId: string, model: unknown) {
  return {
    ...
    query_attribution: tool({ description: '查询远端策略归因治理器、next_action、latest_execution_state、promotion_checklist 和 latest_operations；本地 --no-write 报告需走 CLI/MCP。', inputSchema: z.object({ limit: z.number() }), execute: ({ limit }) => execQueryAttribution(deps, limit) }),
  }
}
