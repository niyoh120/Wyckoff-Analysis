...
describe('attributionOperatorSummary', () => {
  it('uses backend operator summary when present', () => {
    expect(attributionOperatorSummary({
      operations: {
        operator_summary: '下一步=人工复核；作用范围=尾盘；Shadow=2026-07-03 RISK_ON 新增2 移除1',
      },
      actions: [{ target: 'lps' }],
    })).toBe('下一步=人工复核；作用范围=尾盘；Shadow=2026-07-03 RISK_ON 新增2 移除1')
  })
  ...
  it('synthesizes a summary for older attribution reports', () => {
    expect(attributionOperatorSummary({
      ...
      '下一步=shadow 新增组已跑赢移除组。；作用范围=tail_buy_and_funnel_shadow；正式dynamic=allowed；Shadow=2026-07-03 RISK_ON 新增2 移除1；调权=1项',
    )
  })
})
