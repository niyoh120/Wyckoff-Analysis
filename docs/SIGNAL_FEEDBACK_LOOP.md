...
scoped 调权明细，用于每日运营复盘。第一眼先看 `latest_operator_summary` 或
`latest_operations.operator_summary`：它把下一步动作、作用范围、正式 dynamic 是否可晋级、最新
shadow 新增/移除和调权摘要压成同一句话；需要追证据时再看 `latest_execution_state`、
`promotion_checklist`、`latest_operations.latest_shadow` 和 `latest_operations.action_details`。
