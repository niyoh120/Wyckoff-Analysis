...
随着系统功能（如全市场形态复盘、持仓诊断、自动化尾盘策略等）被高频使用，项目基础设施（如多市场行情源、Supabase 数据库、Render 云服务器、AI API）的开销呈指数级增长。日线行情不再长期写入 Supabase 行情缓存，Supabase 主要承载用户配置、持仓、推荐记录、信号反馈和策略观察表。
...
1. **自动推送特权（免部署）**：
   - 每日盘后自动运行 A 股完整漏斗与港股/美股独立扫描；港股/美股结果进入 Web 跟踪复盘表，不进入 AI 审判或飞书推送链路。
   - A 股自动生成威科夫形态复盘报告、AI 三阵营判断及尾盘策略提示，通过**飞书/Telegram**实时推送至您的账户。
2. **多端数据同步（云端专属特权）**：
   - 获得专属的数据库授权与白名单。
