...
| 翌日寄り付き執行 | クロスデイ確認後、承認済み候補を LLM と OMS が再審査し、翌日の寄り付き付近で執行。独立した日中タスクは持たない |
...
**運用:** 日次ファネル=候補と環境、**confirmed** のみ翌日寄り付き付近の買い候補へ進む。NEUTRAL が主戦場、**RISK_ON は新規禁止**。詳細は [OPERATOR_PLAYBOOK.md](OPERATOR_PLAYBOOK.md)。
