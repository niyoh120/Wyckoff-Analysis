...
| 引け値買い戦略 | 取引日の14:40に定時実行し手動補完にも対応、ルールスコアリング + LLM再評価で終盤エントリー対象を選別 |
...
| L5 | AI + OMS 審判 | LLM レビュー、シグナル確認、引け前確認、OMS リスクゲートを通過してから行動 |


**運用:** 日次ファネル=候補と環境、テール=confirmed 後の **BUY** のみ。 NEUTRAL が主戦場、**RISK_ON は新規禁止**。 詳細は [OPERATOR_PLAYBOOK.md](OPERATOR_PLAYBOOK.md)。
...
| 引け値買い戦略 | 月-金 14:40 / 手動補完 | ルールスコアリング + LLM再評価 |
