...
如果最终 synthesis 模型空响应或异常，workflow fallback 会使用真实 step results，并从 `handoff_state` 恢复候选 brief、AI 研报 handoff 和攻防决策 handoff；即使最后一跳 LLM 没产出，用户也能看到已完成步骤、候选代码、证据分、研报状态、攻防下一步，而不是空泛的完成提示。
