...
- `policy_selection` 根据 `ranking_decision` 对最新推荐日输出一组只读候选：未过门槛时仍按 `score_only`，过门槛时展示 promoted ranking 选出的 Top-K；每个 pick 同时带 `action_status`、`quality_factors`、`risk_factors` 和 `next_step`，供 agent / dashboard 快速复盘，不直接改生产推荐。
