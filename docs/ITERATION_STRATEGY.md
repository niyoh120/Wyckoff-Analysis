...
CLI/TUI 的工具结果摘要会把候选行动状态、优先分/动态分/漏斗分、候选影子评级/分和入场质量评级/分直接显示在候选行里；`screen_stocks` 会把同一批质量字段从 `symbols_for_report` 传播到 `top_candidates`、`selection_brief`、`decision_brief` 和 `action_plan`，确保摘要去重后首选候选仍保留质量证据，便于后续模型回合继续引用。`generate_ai_report` 会复用同一套 `candidate_guard_summary`，在研报完成但候选仍有未成熟标签、观察池或阻断状态时，把下一跳限定为组合攻防复核，而不是让 `next_tool` 看起来像无条件买入计划。
