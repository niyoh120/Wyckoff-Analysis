...
如果最终 synthesis 模型空响应或异常，workflow fallback 会使用真实 step results，并从 `handoff_state` 恢复候选 brief、推荐评估、AI 研报 handoff 和攻防决策 handoff；多阶段 handoff 同时存在时，会先给出一行 deterministic 候选结论，再保留每个阶段的关键行并补充候选细节。即使最后一跳 LLM 没产出，用户也能看到首选候选、证据分、禁止直接买入原因、研报状态、攻防下一步，而不是空泛的完成提示。
