...
当模型 router 不可用或输出无效时，harness 只保留一个窄兜底：明显要求“完整选股/候选股/好股票/股票池”并同时要求候选理由、攻防、研报、复核或买卖计划的聊天轮次，会进入 `dynamic_task`，防止“选出好股票”退化成单轮直答；普通持仓、单股诊断、概念解释仍保持 direct。
...
最终 synthesis prompt 会把压缩后的候选 handoff 作为 `priority candidate handoff` 单独提前注入，再附完整 agent results；即使 sub-agent 输出很长导致完整结果截断，候选代码、评分、评级和行动边界仍优先可见。

CLI/TUI 的工具结果摘要会把候选行动状态、优先分/动态分/漏斗分、候选影子评级/分和入场质量评级/分直接显示在候选行里；同一批字段也进入工具结果 preview，便于后续模型回合继续引用。

如果最终 synthesis 模型空响应或异常，workflow fallback 会使用真实 step results，并从 `handoff_state` 恢复候选 brief；即使最后一跳 LLM 没产出，用户也能看到已完成步骤、候选代码、证据分和下一步，而不是空泛的完成提示。
