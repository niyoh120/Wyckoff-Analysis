...
| 方向二：多策略动态分配 | AI 候选配额从静态规则变为数据驱动 | `dynamic_policy.py` 根据信号权重调整 Trend / Accum 配额，支持 `off` / `shadow` / `on`；**NEUTRAL 默认 5/1，RISK_ON 保留 5/1 研究配额但禁止正式执行** | 已落地框架，待 shadow 复盘 |
...
如果最终 synthesis 模型空响应或异常，workflow fallback 会使用真实 step results，并从 `handoff_state` 恢复候选 brief、推荐评估、AI 研报 handoff 和攻防决策 handoff；多阶段 handoff 同时存在时，会先给出一行 deterministic 候选结论，并按同一代码跨阶段合并筛选/推荐评估里的质量证据、研报/攻防里的状态和下一步，再保留每个阶段的关键行并补充候选细节。即使最后一跳 LLM 没产出，用户也能看到首选候选、证据分、禁止直接买入原因、研报状态、攻防下一步，而不是空泛的完成提示。
...
跨周期 robust ranking 选锚点，再寻找同一 portfolio style 下仅一个参数相邻变化的组合。锚点须完整覆盖
`recent_6m`、`bull_2020`、`bear_2022` 且现金收益全部为正；缺周期或邻居少于 2 个时为 `review`；邻居覆盖充足时，至少 50% 邻居也须三周期全部为正，
否则判为参数孤岛并输出 `fail`。产物为 `parameter_stability.json`，指定 hypothesis 时还会生成并
关联 `stability_evidence.json`。

Backtest Grid 还输出 `walk_forward_validation.json`。它按回测区间结束时间排序，在训练窗口选择最优持有/
退出组合，并在紧随其后的窗口原样评估，禁止测试期重新选参。当前参数矩阵尚未覆盖 SOS/Spring/LPS 的
触发阈值，因此该产物只证明现有持有与退出参数的样本外稳定性，不替代触发器阈值专项 walk-forward。

触发器专项验证由 `strategy_compare` 承担：A/B/C/D/E 五组不改变数据快照、候选容量、入场成交与退出
参数，只切换 UTAD、regime 触发阈值、Creek/LPS 和跨信号时序能力。报告要求至少两个共同周期、相对 A
组至少在两个周期真实改变 `(signal_date, code)` 交易集合、胜出周期过半、平均收益增量为正且最大回撤恶化不超过 2 个百分点；零处理暴露标记为 `no_effect`。满足条件仍只进入推广评审，不自动改写
生产开关。
