...
最终 synthesis prompt 会把压缩后的候选 handoff 作为 `priority candidate handoff` 单独提前注入，并按 handoff 类型保留最新值，避免多 step 累积状态重复刷屏；再附完整 agent results。即使 sub-agent 输出很长导致完整结果截断，候选代码、评分、评级和行动边界仍优先可见。
